...
so$cluster_subpopulation <- plyr::mapvalues(
  ...
  to = c("Homeostatic", "Homeostatic", "Disease_associated", "Reactive", "Homeostatic", "Disease_associated", "Homeostatic", "Metallothionein", "Metallothionein", "Inflammatory", "Intermediate", "Doublets (ODC)", "Doublets (Neurons)", "Inflammatory", "Doublets (Micro)", "Metallothionein"))
...
subpopulation_markers <- read.table(file.path(marker_dir, "astrocyte_markers.txt"), sep="\t", row.names = NULL, header = F)
